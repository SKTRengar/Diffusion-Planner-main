...
from diffusion_planner.model.module.attention import multihead_attention
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, cross_c, y, attn_mask):
        ...
        x = x + gate_msa.unsqueeze(1) * multihead_attention(
            self.attn, modulated_x, modulated_x, modulated_x, key_padding_mask=attn_mask
        )

        modulated_x = modulate(self.norm2(x), shift_mlp, scale_mlp)
        x = x + gate_mlp.unsqueeze(1) * self.mlp1(modulated_x)

        x = multihead_attention(self.cross_attn, self.norm3(x), cross_c, cross_c)
